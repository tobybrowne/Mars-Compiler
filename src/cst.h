...
bool validPattern(std::vector<std::variant<TokenType, CstNonTerminal>> pattern, CstNode* rootTreeNode, size_t& inputTokenPtr, const std::vector<Token>& tokenStream) {
    size_t savedPtr;
    bool valid;
    ...
    for (std::variant<TokenType, CstNonTerminal>& component : pattern) { // by reference is more efficient.
        ...
        if (!newTreeNode->isToken) {
            ...
            for (std::vector<std::variant<TokenType, CstNonTerminal>>& possiblePattern : patternList[std::get<CstNonTerminal>(component)]) {
                ...
                if (validPattern(possiblePattern, newTreeNode, inputTokenPtr, tokenStream)) {
                    valid = true;
                    break;
                }
                else {
...
CstNode* generateCST(bool debugMode, std::vector<Token> tokenStream) {
    ...
    if (!validPattern(patternList[CstNonTerminal::COMPOUND_STMT][0], rootNode, inputTokenPtr, tokenStream)) {
        return NULL;
    }
    return rootNode;
}
